...
RSpec.describe Customer, type: :model do
  ...
  it "is not valid if the phone number is not 10 chars" do
    subject.phone = "123456789"  # Phone number with less than 10 characters
    expect(subject).to_not be_valid
    subject.phone = "12345678901"  # Phone number with more than 10 characters
    expect(subject).to_not be_valid
  end

  it "is not valid if the phone number is not all digits" do
    subject.phone = "12345678ab"  # Phone number with non-digit characters
    expect(subject).to_not be_valid
  end

  it "is not valid if the email address doesn't have a @" do
    subject.email = "jsmithsample.com"  # Invalid email format
    expect(subject).to_not be_valid
  end
